...
"""asd"""
dates = ['2018-5','2018-6','2019-6', '2019-7', '2019-8','2020-5', '2020-6', '2020-7']
page = 'https://www.wunderground.com/history/monthly/hu/debrecen/LHDC/date/'

list_actual = scraper(page, dates)

# concat all dataframes in one
df = pd.concat(list_actual, sort =False)
...
df.to_csv('wheather_In_One.csv')

csvdata = pd.read_csv('wheather_In_One.csv')
csvdata.drop(["Unnamed: 0"], axis = 1, inplace = True)
csvdata.drop(["Temp_max"], axis = 1, inplace = True)
csvdata.drop(["Temp_min"], axis = 1, inplace = True)
csvdata.drop(["Dew_max"], axis = 1, inplace = True)
csvdata.drop(["Dew_avg"], axis = 1, inplace = True)
csvdata.drop(["Dew_min"], axis = 1, inplace = True)
csvdata.drop(["Wind_min"], axis = 1, inplace = True)
csvdata.drop(["Hum_min"], axis = 1, inplace = True)
csvdata.drop(["Hum_max"], axis = 1, inplace = True)
csvdata.drop(["Wind_max"], axis = 1, inplace = True)
csvdata.drop(["Pres_max"], axis = 1, inplace = True)
csvdata.drop(["Pres_min"], axis = 1, inplace = True)
csvdata.drop(["Pres_avg"], axis = 1, inplace = True)
csvdata.drop(["Date"], axis = 1, inplace = True)
csvdata.drop(["Jul"], axis = 1, inplace = True)
csvdata.drop(["Jun"], axis = 1, inplace = True)



csvdata['Temp_avg'] = csvdata['Temp_avg'].apply(fahrenheit_to_celsius)
csvdata['Precipitation'] = csvdata['Precipitation'].apply(inches_to_cm)


csvdata.to_csv('Clear_Data_20192020_1.csv')
